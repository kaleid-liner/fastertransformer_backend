...
def lines2inputs(FLAGS, lines, tokenizer, max_length):
...
def predict(client, input, chatbot, max_length, top_p, temperature):
    chatbot.append((parse_text(input), ""))
    inputs, input_lengths = lines2inputs(FLAGS, [input], tokenizer, max_length)
...
result_queue = Queue()
client = create_inference_server_client(FLAGS.protocol,
                                        FLAGS.url,
                                        concurrency=1,
                                        verbose=verbose)
...
with gr.Blocks() as demo:
    ...
    submitBtn.click(partial(predict, client), [user_input, chatbot, max_length, top_p, temperature], [chatbot],
                    show_progress=True)
